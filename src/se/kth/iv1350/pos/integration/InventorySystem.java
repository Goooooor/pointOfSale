...
public class InventorySystem {
	 ...
	 * @throws ItemNotFoundInInventoryException When an item can not be found with
	 *                                          the entered itemID.
	 */
	public ItemDTO getItemInformation(int itemID)
			throws ItemNotFoundInInventoryException, UnreachableDatabaseException {
	...
	public void updateInventory(Sale sale) {
		...
		for (int i = 0; i < sale.getItems().size(); i++) {
			...
			for (int n = 0; n < inventory.size(); n++) {
				if (updatedItem.getItemDescription().equals(inventory.get(n).getItemDescription())) {
					inventory.remove(n);
					inventory.add(n, updatedItem);
				}
			}
		}
	}
